<script lang="ts">
  ...
  onMount(async () => {
    ipportpairs = await scanLocalNetworkOnlineHostsByPort(1566);
  });

  async function check() {
    loading = true;
    ipportpairs = await scanLocalNetworkOnlineHostsByPort(port, keyword);
    loading = false;
  }
  $: {
...
<h2 class="text-2xl">Scan Online CrossCopy Service</h2>
